...
function Neuerwerbung(Prefix, YoderM) {
    ...
    const linkPart1 = "https://uzb.swisscovery.slsp.ch/discovery/search?tab=41SLSP_UZB_DN_and_CI&search_scope=DN_and_CI&vid=41SLSP_UZB:UZB&lang=en&offset=0&query=any,contains,";
...
$(function(){
    let anchor = $('#test-eng'); 
    let ancElements = $('#test-eng .link');
    

    ancContents = '';
    
    $.each(ancElements,function(){
        
        let prefix = $(this).attr('class').split(" ")[1];
        let text = $(this).attr('id').replaceAll("-", " ");
        console.log(text);
